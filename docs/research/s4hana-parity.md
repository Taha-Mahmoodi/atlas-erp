...
**Reconciliation (August 2026, PLAN 17.2).** Every row was re-verified against the shipped v1 code. Built capabilities cite their PLAN task as **Done (PLAN x.y)**; two statuses changed during the build (output management Partial → Out of scope: no document rendering shipped; metadata-driven UI extensibility Full → Partial: the def registry shipped but no entity stores values and no UI surfaces them); one row was added during the build (CRM-lite, PLAN 12.1, taking the map to 140 capabilities); and known limitations documented by v1 QA (issues #163–#166) plus the deliberate RETURN_TO_VENDOR 422 are noted inline on their rows.
...
| **Self-service query creation and export (Query Browser, custom analytical queries)** | Full | reporting | Known limitation (QA #166): the results grid and CSV export show wire column names (`sum_total_amount`) instead of the entity descriptor's display labels; later path is mapping headers through the descriptor catalog. |
