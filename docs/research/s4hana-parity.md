...
| **Multi-currency accounting and foreign currency valuation** | Partial | finance | Atlas v1 plans only transaction + functional currency with rates table and realized/unrealized FX accounts; no group/additional parallel currencies or consolidation-style currency translation. Later: Add extra currency amount columns per journal line plus a translation run, keeping the same rates-table and FX-account machinery. |
...
| **Sales quotations and order management** | Full | sales | — |
| **Available-to-promise (ATP) and backorder processing** | Partial | sales | Atlas v1 plans a simple check against on-hand plus on-order with manual backorders, but none of aATP: no product allocation/quotas, alternative plant/substitution confirmation, supply protection, or prioritized mass BOP runs. Later: Add a scheduled re-promising (backorder run) job and allocation buckets on top of the existing availability query. |
| **Shipping and outbound deliveries** | Partial | sales | Atlas v1 plans delivery documents with partial shipments and stock issue, but no picking/packing/handling units, shipping point or route determination, wave processing, or warehouse/transportation integration. Later: Layer pick lists, packing/handling units, and shipping-point/route rules onto the delivery document later. |
...
| **Credit management** | Partial | sales | Atlas v1 plans only a static credit-limit block at order confirmation; no exposure aggregation across open orders/deliveries/receivables, no rechecks at delivery, and no scoring or release workbench. Later: Introduce a finance-owned exposure ledger and a blocked-documents release queue, rechecking at delivery creation. |
...
| **Equipment and functional locations (technical objects)** | Partial | maintenance | A flat equipment register is planned, but functional-location hierarchies, install/dismantle, and structure-based reporting are explicitly excluded. Later: Add a functional location entity with hierarchy codes and equipment installation/dismantle links. |
...
| **Corrective maintenance orders** | Full | maintenance | — |
| **Preventive maintenance plans and scheduling** | Partial | maintenance | Only fixed interval-based (time) preventive orders are planned; counter/performance-based and strategy plans are impossible without measurement points. Later: Once counters exist, extend the scheduler with performance-based cycles and packaged strategy plans. |
...
| S/4HANA capability | Atlas v1 | Atlas module | Notes |
|---|---|---|---|
| **Employee master data (Personnel Administration)** | Full | hr | Later: Add date-effective history and formal hire/transfer/terminate actions on the existing employee entity. |
| **Organizational management (org units, positions, org chart)** | Full | hr | Later: Extend later with date-effective org structures and job catalogs if needed. |
...
| **Compensation management (pay structures and review cycles)** | Partial | hr | Atlas v1 stores only static masked compensation fields on the employee; no pay-grade structures or compensation review cycles. Later: Add pay-grade/salary-band tables and a periodic comp review workflow over the masked fields. |
