...
**Reconciliation (August 2026, PLAN 17.2).** Every row was re-verified against the shipped v1 code. Built capabilities cite their PLAN task as **Done (PLAN x.y)**; two statuses changed during the build (output management Partial → Out of scope: no document rendering shipped; metadata-driven UI extensibility Full → Partial: the def registry shipped but no entity stores values and no UI surfaces them); one row was added during the build (CRM-lite, PLAN 12.1, taking the map to 140 capabilities); and the v1 QA findings (issues #163–#166) plus the deliberate RETURN_TO_VENDOR 422 are noted inline on their rows — each such note says whether it is still an open limitation or which PR fixed it, so a row never outlives the behaviour it describes.
...
| Sales & Distribution (SD) | 2 | 7 | 4 | 13 |
...
| Cross-cutting platform concepts | 5 | 2 | 4 | 11 |
| Fiori UX & Embedded Analytics | 3 | 6 | 3 | 12 |
| Industry Solutions approach | 2 | 5 | 3 | 10 |
| **Total** | **44** | **49** | **47** | **140** |
...
| **Multi-currency accounting and foreign currency valuation** | Partial | finance | **Done (PLAN 4.3–4.6):** transaction + functional currency, a rates table (SPOT/CLOSING, direct-or-inverse lookup), posting-time translation with frozen functional amounts (largest-remainder residual balancing), purpose-keyed FX posting defaults, an **unrealized**-FX revaluation run with auto-reversal, and — landed with AP/AR via one shared clearing helper (D-031) — **realized** FX posted at payment/receipt clearing. **Bounded out of v1:** group/additional parallel currencies; consolidation-style translation; and **per-open-item** revaluation granularity — v1 revalues the per-account monetary foreign balance (accounts flagged `is_monetary` with a foreign `currency_code`), not individual uncleared AP/AR items. Later: add per-line extra currency columns + open-item-granular revaluation, reusing the same rates-table and FX-account machinery. |
| **Accounts Payable (vendor invoices, payment runs, aging)** | Full | finance | **Done (PLAN 4.5):** vendor bills posting to AP-control journals, payment clearing with realized FX, batch payment runs (202 + background job), and the AP aging projection; bank payment file formats (ISO 20022/NACHA) and withholding tax did not make v1. Later: Layer payment-media file generation and withholding tax codes onto the existing payment-run engine. |
| **Accounts Receivable (customer invoices, receipts, dunning, aging)** | Full | finance | — |
| **Asset Accounting (parallel valuation, full lifecycle)** | Partial | finance | **Done (PLAN 4.10)** as "asset accounting lite": register plus straight-line and declining-balance depreciation runs posting journals; no parallel depreciation areas, transfers, retirements with gain/loss, revaluation, or assets under construction. Later: Extend the asset register with lifecycle events (transfer/retire/AuC settle) and per-ledger depreciation areas reusing the existing depreciation-run poster. |
| **Bank and Cash Management** | Partial | finance | **Done (PLAN 4.9):** bank reconciliation via CSV statement import with automatic match suggestions (confirm/reject) and line clearing; no standard bank formats (MT940/CAMT), no auto-clearing rules engine, no cash positioning or liquidity forecasting. Later: Add MT940/CAMT.053 parsers feeding the same import pipeline, then a cash-position report over bank-clearing accounts. |
| **Tax handling (tax codes, determination, compliance reporting)** | Partial | finance | **Done (PLAN 4.4):** line-level configurable tax codes (rate, jurisdiction, inclusive/exclusive) with exact-allocation document tax and OUTPUT/INPUT directions consumed by Sales and Procurement — the calculation core — but no withholding tax, no statutory tax return reports, and no external tax engine integration. Later: Build tax-return reports as queries over tax-coded journal lines and add a pluggable external tax-calculation interface. |
...
| **Product cost planning (standard cost estimates)** | Partial | manufacturing | **Done (PLAN 8.2)** as manufacturing-fed product costing (actual component costs to WIP, finish-to-stock valuation); no BOM/routing cost-rollup engine, cost component split, or standard price release/costing-run machinery. Later: Add a cost-rollup service over BOM/routing data that produces standard costs with a component split, used to value finished-goods receipts and compute variances. |
| **Cost object controlling (WIP, variances, settlement)** | Partial | manufacturing | **Done (PLAN 8.2)** for the core flow — WIP journals and finish-to-stock postings, which matches S/4HANA's modern event-based direction — but variance decomposition (input price/quantity, resource usage, lot size), scrap analysis, and settlement profiles were not built. Later: Add variance categorization computed from journal deltas at order close, posting categorized variance lines to the ledger and the margin report. |
| **Margin analysis / profitability analysis (CO-PA)** | Partial | reporting | **Done (PLAN 4.8)** as a margin-by-item report projected from the ledger — the right account-based architecture — but only the item dimension, and backend-only: the statements UI ships the four core statements, while the margin and cost-center report pages were a documented frontend cut (PLAN 15.4 slice 4). Customer/region/channel segments, COGS split by cost component, top-down distribution, and realignments are not built. Later: Widen journal lines with additional market-segment attributes (customer, region, channel) derived at posting time and add segment-level margin reports over the same ledger projection. |
| **Allocations and assessments (universal allocation)** | Full | finance | **Done (PLAN 4.7):** allocation rules on cost centers and profit centers posting balanced CO entries through the journal via tracked allocation runs — the practitioner-essential core; allocation to margin-analysis segments and plan-data allocation are the main S/4 extras not built. Later: Add margin-segment-context and plan-data allocation cycles on the same rule engine. |
...
| **Goods receipt against purchase order** | Full | inventory | **Done (PLAN 6.3):** GR against a sent PO posts the stock RECEIPT move and the GR/IR clearing journal via the event bus, with partial receipts advancing the PO to PARTIALLY_RECEIVED and over-receipt rejected; stock/valuation postings are owned by the inventory module. |
| **Invoice verification and 3-way match (MM-IV/LIV)** | Full | procurement | **Done (PLAN 6.4):** 3-way match (invoice vs PO price, vs GR quantity) with a tenant-wide price/quantity tolerance percentage, EXCEPTION on breach with an explicit override, and posting that auto-creates+posts the AP vendor bill via the event bus; AP posting itself lives in finance. Later: Per-vendor tolerance groups, invoice-release workflow, and ERS (evaluated receipt settlement) are natural follow-ons. |
...
| **Stock types and special stocks (unrestricted / QI / blocked; consignment, subcontracting, in-transit)** | Partial | inventory | Atlas v1 tracks quantity per warehouse/bin via stock moves with no stock-status dimension (QI/blocked) and no special-stock ownership categories such as consignment or subcontractor stock; the closest v1 gets is quality's BLOCK disposition (PLAN 9.1), which quarantines rejected stock by TRANSFER to a designated blocked/QI bin — bin-level, not a status. Later: Add status and ownership dimensions to stock quants/move lines, with status-change moves reusing the existing move ledger. |
...
| **Material valuation: moving average, standard price, FIFO (Material Ledger)** | Partial | inventory | **Done (PLAN 5.3):** moving average AND FIFO per item (defaulted from the item category, then owned by the item — D-020) with event-driven COGS posting; standard costing with variance (price-difference) postings, which manufacturers treat as essential, was not built. Later: Add a standard-cost method to the per-item costing engine, posting purchase/production variances to dedicated variance accounts. |
...
| S/4HANA capability | Atlas v1 | Atlas module | Notes |
|---|---|---|---|
| **Customer master (Business Partner)** | Partial | sales | **Done (PLAN 7.1):** customer master with customer groups, currency, payment terms, credit limit and status, but not S/4HANA's multi-role business partner model with distinct partner functions (separate ship-to/payer/bill-to per document). Later: Evolve the customer record into a party model with role assignments and per-order partner function overrides. |
| **Pricing and condition technique** | Partial | sales | **Done (PLAN 7.1):** condition-style price lists by currency/customer group/date window with priority, per-item lines carrying a single `min_quantity` floor, plus per-line discounts — but not the generalized access-sequence/pricing-procedure engine, multi-tier quantity scales, freight/tax condition types, or price-approval workflows. Later: Generalize price tables into condition tables with a pluggable access-sequence resolver and an ordered pricing procedure. |
...
| **Billing and invoicing** | Partial | sales | **Done (PLAN 7.4):** per-order-line billing of delivered-not-yet-invoiced quantities that posts the finance AR invoice via the event bus (docflow-linked, never a cross-module FK), with credit notes issued by RMA returns — but not collective billing runs, invoice split/merge rules, milestone or recurring billing plans, or preliminary billing. Later: Add a collective billing-due-list job with split/merge rules, then billing plans as a follow-on. |
...
| **Returns processing (RMA / Advanced Returns Management)** | Full | sales | **Done (PLAN 7.4):** RMA anchored to the order, capped at invoiced-minus-returned (enforced at create and re-checked at post), posting the stock receipt at book cost and a credit note reversing COGS and revenue in one transaction. Later: Inspection dispositions, refund-determination rules, and vendor-return chains can be added on the RMA later. |
...
| **Order confirmations (operation-level time/quantity recording)** | Partial | manufacturing | **Done (PLAN 8.2)** at order level only (issue to WIP, finish to stock, WIP journals); per-operation confirmations with actual time capture and scrap recording were not built. Later: Add a confirmation entity keyed to routing operations that posts actual times/quantities into the existing WIP journals. |
| **MRP and MRP Live** | Partial | manufacturing | **Done (PLAN 8.3):** the core deterministic run (sales-order demand + reorder points exploded level-ordered against supply, producing planned orders with firm/convert/cancel); exception/rescheduling messages, MRP areas, multi-plant scope, net-change planning, and MRP-Live-class in-memory performance are not built. Later: Layer exception messages, pegging, and net-change/multi-plant runs onto the same planning engine without schema changes. |
| **Capacity planning (evaluation and leveling)** | Partial | manufacturing | **Done (PLAN 8.3)** as capacity evaluation only (rough load vs available hours per work center); interactive leveling, dispatching, and finite scheduling were not built. Later: Add a dispatching board that re-dates order operations against a finite work-center calendar using the existing load data. |
...
| S/4HANA capability | Atlas v1 | Atlas module | Notes |
|---|---|---|---|
| **Inspection lots** | Partial | quality | **Done (PLAN 9.1):** GR-origin lots only, auto-created via the event bus from a posted goods-receipt line's `requires_inspection` flag; production/in-process, delivery, and manual lot origins plus sampling/sample-size logic are not built. Later: Extend the existing lot object with additional origins (production order, delivery, manual) and sampling procedures. |
...
| **Usage decisions with stock disposition** | Partial | quality | **Done (PLAN 9.1):** binary accept/reject with a stock move — reject requires a disposition of SCRAP (ADJUSTMENT-out plus write-off journal at book value) or BLOCK (value-neutral TRANSFER to the designated blocked/QI bin); RETURN_TO_VENDOR is declared in the catalog but deliberately rejected with a 422 in v1 — it needs the vendor-return/credit chain. Coded UD catalogs, quality scores, and split-quantity postings are not built. Later: Add UD code catalogs, multi-bucket quantity split posting, and the vendor-return chain behind RETURN_TO_VENDOR on top of the existing accept/reject action. |
...
| **Procurement and expense postings to projects** | Partial | projects | **Done (PLAN 11.1):** actual purchase and expense postings to WBS, but PR/PO commitment tracking and project stock / project MRP were not built. Later: Derive commitment records from open purchase orders assigned to WBS, then add project-stock segments in inventory. |
...
| **Output management (determination rules, channels, form templates)** | Out of scope | — | Cut during the build: the "basic document rendering" once planned never shipped — v1 renders no document at all (APIs return JSON; the only export is the report builder's streaming CSV), and the rule-based determination framework, multi-channel dispatch (email/EDI), template management, and output status/retry queue were never started. Later: Add template-based PDF/email rendering first, then a rule-driven output determination service (document attributes → channel + template + recipient) with pluggable channel adapters and a retryable output queue. |
| **Number ranges (document numbering)** | Full | core | **Per-fiscal-year ranges shipped (#209, D-079, PR #214):** each sequence keeps one gapless series PER YEAR in `core_number_sequence_counters`, so a document backdated across a year boundary claims that year's next number instead of resetting the live counter — which is what the single-counter design did, corrupting the sequence and 500-ing every later document of that type. Non-resetting sequences store year 0 (NULLs compare distinct in a UNIQUE index on both dialects). Known limit (#216, open): the migration carries across only the year each sequence was serving, so a tenant already bitten before the upgrade keeps a gap in the OTHER year. Later: add external number assignment if conversion/legal scenarios demand it. |
...
| **Extensibility framework (custom fields, key-user and side-by-side extensions)** | Out of scope | — | The D-016 foundation shipped in core (`core/custom_fields.py`: custom-field def registry + typed validator + JSONB column helper, with industry templates registering defs at provisioning), but no v1 entity opts into the values column and no UI surfaces custom fields — so end-to-end extensibility remains out; v1 effectively ships fixed schemas. Later: Opt core entities into the existing JSONB column + validator, surface registered defs in the form builder, plus webhook/event hooks per document type. |
| **Workflow engine (flexible approval workflows)** | Out of scope | — | No generic workflow engine shipped; the build did land data-driven value-threshold approval rules for requisitions/POs (PLAN 6.2) and header-level approvals in HR (leave requests, timesheets), but these are per-module, single-step gates — not a configurable cross-module release-strategy engine. Later: Add a generic approval service (document type + condition → ordered approver steps) driving a status field that blocks posting until approved. |
...
| **Fiori design system and UI component library (Horizon, SAPUI5 controls)** | Partial | frontend | **Done (PLAN 15.2)** as the focused in-house kit (data grid, form builder, kanban, dashboard cards, document-flow viewer), not a full design system with theming tokens, accessibility standards, and the breadth of SAPUI5 controls. Known limitation (QA #164): FormBuilder renders `noValidate` and enforces no required fields client-side — empty required submits rely on backend 422 banners; later path is enforcing required flags at submit with field-level errors. Later: Grow the kit into a documented design system: theme tokens, a11y audit, and new controls added as app screens demand them. |
...
| **Self-service query creation and export (Query Browser, custom analytical queries)** | Full | reporting | QA #166 **fixed (PR #198)**: the results grid and the CSV export take their headers from one backend list, `ReportResult.column_labels` — the registry's display label for a plain column and a composed `"Sum of Total"` for an aggregate — instead of the wire column names (`sum_total_amount`); a caller-supplied aggregate `alias` is used verbatim as its own label. |
| **Insight-to-action and intent-based cross-app navigation** | Partial | frontend | Atlas ships the document-flow viewer component (PLAN 15.2), but no generic deep-link/intent framework wiring dashboard cards and report rows to the owning app screens. Later: Define per-entity URL route conventions (deep links) and wire dashboard cards, KPI tiles, and report rows to them. |
...
| **Industry-specific master data models** | Partial | industry | Templates register typed custom-field DEFINITIONS per entity (metadata registry, PLAN 14.1) — but the values column is not yet on any core entity, and Atlas v1 ships no first-class industry entities such as article variant matrices, store hierarchies, or a patient demographics object. Later: Ship industry entities as optional modules built on the metadata registry, starting with retail article/site. |
...
| **Preconfigured industry best-practice content applied at provisioning** | Full | industry | Known limitation (QA #165): the onboarding wizard seeds the first admin with only 5 admin.* permissions, so a fresh tenant admin cannot read the COA/tax codes its own template just instantiated; later path is deciding the intended default role for a fresh tenant admin and seeding permissions accordingly. |
| **Metadata-driven field extensibility surfaced across the UI** | Partial | core | Downgraded at reconciliation: the def registry, typed validator, and JSONB column helper shipped in core (D-016) and templates register field defs at provisioning (PLAN 14.1), but no v1 entity carries the `custom_fields` values column and nothing surfaces registered defs in the UI. Later: opt the first master-data entities into the column and render registered defs through the form builder. |
