...
## D-078 — Background-job durability: the stale-job sweeper and its precondition
...
2. **The runner claims its row, and only PENDING is ever re-dispatched.** `_run_handler` transitions PENDING → RUNNING with a CONDITIONAL update and returns without touching the handler if it loses. Since nothing ever moves a row back out of RUNNING, that one conditional update gives at-most-once execution outright — which is what makes reclaiming a PENDING row safe when the original asyncio task was merely queued behind `MAX_CONCURRENT_JOBS` rather than dead. A stale **RUNNING** row is therefore never re-dispatched: it is marked FAILED for a human. Re-dispatching it would run the same payload *concurrently* with a possibly-live handler, and every idempotency guard in Atlas is read-then-write with no lock — `run_payment_batch` selects bills with `open_amount > 0` before either transaction commits, so the same vendor bill gets paid twice; FX revaluation reverses only a prior COMPLETED run; the depletion guard reads the already-issued items before the first run's moves commit. Only two handlers have a DB-level backstop. Sequential re-run safety is a precondition, not a defence against concurrency.
...
**Thresholds and bounds.** PENDING reclaims after 10 minutes; RUNNING is given up on after 2 hours — nothing distinguishes "the process died" from "this MRP run is slow" except elapsed time, so that window must exceed the slowest legitimate handler by a wide margin, and the terminal write is conditional on the row still being RUNNING so a slow handler finishing mid-sweep is not overwritten. 50 rows per tick keeps a post-outage backlog from scheduling thousands of tasks on an already-unhealthy system. **There is deliberately no attempt ceiling.** `core_jobs.attempts` (migration 0049) counts sweeps, not executions — a handler that raises is set FAILED by the runner and never swept again — so a ceiling on it could only ever fire on a job that keeps failing to get *started*, most often one merely queued behind the concurrency cap; abandoning that would mark FAILED a job no runner ever touched. The bound comes from the RUNNING rule instead: a job that starts and does not finish is FAILED once, and stays FAILED. `attempts` is diagnostics only.
...
**Rejected alternatives.**
- A heartbeat / lease column on `core_jobs`, so a RUNNING row could be told "dead" from "slow" precisely, and a dead one safely re-dispatched: correct, and it is the real upgrade path. It was not built because it makes every handler write on a timer, and the cheap answer — never re-dispatch RUNNING, just fail it — costs only manual resubmission on a path that is already rare (a crash catches at most `MAX_CONCURRENT_JOBS` rows RUNNING; the queued ones are PENDING and recover automatically).
...
- A genuinely slow live runner that crosses the 2-hour line has its row marked FAILED while it is still working. If it then succeeds, the terminal write is lost (the conditional UPDATE stops the reverse — a job that COMPLETES first is not overwritten) and a human is told to investigate something that worked. That is the deliberate trade against re-dispatching it: a spurious FAILED row is recoverable, a concurrently duplicated payment run is not. A heartbeat column would remove the guesswork and was not built.
