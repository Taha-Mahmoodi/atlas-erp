...
class Tenant(UuidPKMixin, TimestampMixin, Base):
    ...
    and the D-007 filter must never apply to resolving them."""
...
class TenantSetting(UuidPKMixin, TenantMixin, TimestampMixin, Base):
    """Per-tenant configuration/onboarding flags (consumed by PLAN 14.2 provisioning);
    also the first real tenant-scoped table the D-007 guard tests run against."""
