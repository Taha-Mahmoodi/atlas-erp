...
def api_key_factory(db_session: AsyncSession) -> ApiKeyFactory:
    ...
    async def _make(
        ...
        *,
        ...
        full, digest = mint_api_key(tenant_ref or principal.tenant_slug)
...
async def test_key_lookup_is_filtered_by_the_tenant_in_the_key(
    ...
    full = await api_key_factory(
        attacker, scopes=[ADMIN_USER_MANAGE], tenant_ref=victim.tenant_slug
    )
...
@pytest.mark.parametrize(
    "raw",
    [
        "atk_victim_forgedsecret",  # no such tenant
        "atk_acme_forgedsecret",  # real tenant, secret matches no row
    ],
)
async def test_forged_key_is_rejected(
    client: AsyncClient, admin_user: ProvisionedUser, raw: str
) -> None:
    """Both failures return the same 401 envelope, so the response is not an oracle for
    whether the tenant ref exists."""
