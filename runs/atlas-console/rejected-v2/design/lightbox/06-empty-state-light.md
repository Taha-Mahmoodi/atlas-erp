...
decorative element to the plainest register in the set.
...
border-radius. Fill `color-mix(in oklab, oklch(0.98 0.01 290) 60%, transparent)`,
`backdrop-filter: blur(20px) saturate(140%)`, border `color-mix(in oklab, white 70%,
oklch(0.7 0.05 290) 30%)`. Left to right, 16px outer padding, 8px gaps: 24×24 brand glyph +
"Atlas" wordmark (15px/600) → search field, flex-grow, placeholder "Search or press / to jump"
(14px/500, command-bar-label style, on its own text scrim) → quick-create icon-button
`44×44px` (`+`, `aria-label="Quick create"`) → notifications icon-button `44×44px` (bell,
`aria-label="Notifications"`). Every text run on this bar sits on its own scrim:
`background: color-mix(in oklab, oklch(0.98 0.01 290) 78%, transparent)`, per the concept's
glassmorphism fix — flagged for build-time re-measurement against the busiest scrolling row,
not assumed passing. Item creation lives here (quick-create), not duplicated as a second
button in the content header below.
...
"Clear filters" in `accent-ink`, no underline at rest, underline on `:hover`/`:focus-visible`.
...
`8px 16px` padding, `8px` radius, above everything including the glass bar. Copy: "Skip to
items list" — targets the h1.

**Focus ring** (skip link, rail nav items, bar's search/quick-create/notification controls,
each chip's dismiss glyph, "Clear filters"): `2px solid accent-ink`, `2px` offset,
`:focus-visible` only.
...
| Chip fill, active-nav-row fill | row-alt | `oklch(0.965 0.006 290)` | text-primary | ~15:1 |
| Table header, chip text, footer meta | text-secondary | `oklch(0.47 0.015 290)` | on substrate | ~6.3:1 |
| Rail border, chip border, table header rule | hairline | `oklch(0.90 0.008 290)` | — sub-3:1, decorative only, same caveat as the concept's own note |
| "Clear filters" link, focus ring, skip-link bg | accent-ink | `oklch(0.43 0.15 290)` | on substrate | ~7.2:1 (measured "on white" in the source table; substrate here is effectively the same lightness) |
| Glass command bar fill | glass panel | `color-mix(in oklab, oklch(0.98 0.01 290) 60%, transparent)` + `blur(20px) saturate(140%)` | text via inner scrim | flagged for build-time re-measurement (RISK 1) |
| Command-bar text scrim | — | `color-mix(in oklab, oklch(0.98 0.01 290) 78%, transparent)` | sits behind every bar text run | — |
...
2. **Different one-line message**, same position (`y:556`, same 14px/450, same inline-link
   ...
   "Nothing logged yet." in `text-primary`, "Log the first item" in `accent-ink`, same
