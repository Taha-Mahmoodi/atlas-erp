/**
 * The line editor for a draft requisition (module-specific composite, STRUCTURE §4). Mirrors
 * finance's BillLinesEditor. estimated_unit_cost is optional (lines with no estimate
 * contribute 0 to the approval-threshold total the backend computes at submit time) — the
 * approval gate is informational here, not something this editor pre-computes client-side.
 */

import type { Item, Uom } from "@/modules/inventory/types";
import type { RequisitionLineCreate } from "@/modules/procurement/types";

export interface RequisitionLinesEditorProps {
  lines: RequisitionLineCreate[];
  items: Item[];
  uoms: Uom[];
  onChange: (lines: RequisitionLineCreate[]) => void;
}

function emptyLine(): RequisitionLineCreate {
  return { item_id: "", quantity: "", uom_id: "", currency_code: "USD" };
}

export function RequisitionLinesEditor({ lines, items, uoms, onChange }: RequisitionLinesEditorProps) {
  const updateLine = (index: number, patch: Partial<RequisitionLineCreate>) => {
      ...
      <table className="w-full border-collapse text-[13px]">
        <thead>
          <tr className="border-b border-line text-left text-[11px] font-semibold uppercase tracking-[0.02em] text-ink-muted">
            ...
            <th className="w-32 py-2 pr-2 text-right">Est. unit cost</th>
            <th className="w-20 py-2 pr-2">Currency</th>
            ...
            <tr key={index} className="border-b border-line last:border-b-0">
              ...
              <td className="py-1.5 pr-2">
                <input
                  ...
                  onChange={(event) =>
                    updateLine(index, { estimated_unit_cost: event.target.value || null })
                  }
                  className="w-full rounded-control border border-line bg-surface px-2 py-1 text-right text-sm tabular-nums"
                />
              </td>
              <td className="py-1.5 pr-2">
                <input
                  ...
                  onChange={(event) => updateLine(index, { currency_code: event.target.value.toUpperCase() })}
                  className="w-full rounded-control border border-line bg-surface px-2 py-1 text-sm"
                />
              </td>
