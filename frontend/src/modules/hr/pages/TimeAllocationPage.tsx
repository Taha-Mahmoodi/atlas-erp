    ...
    <div className="mx-auto max-w-3xl">
      <header className="mb-6">
        <p className="text-[12px] text-ink-muted">
          <Link to="/hr">HR</Link> / <span className="text-ink">Time allocation</span>
        </p>
        <h1 className="mt-1.5 text-[22px] font-[650] tracking-[-0.01em] text-ink">Time Allocation</h1>
        <p className="mt-1 text-[13px] text-ink-muted">Approved hours grouped by cost center or project.</p>
      </header>
      ...
      <div className="mt-4 rounded-card border border-line bg-surface px-[18px] py-4 shadow-card">
        {!dateFrom || !dateTo ? (
          <p className="text-[13px] text-ink-muted">Pick a date range to run the report.</p>
        ) : report.isPending ? (
          <p className="text-[13px] text-ink-muted">Loading…</p>
        ) : rows.length === 0 ? (
          <p className="text-[13px] text-ink-muted">No approved hours in this range.</p>
        ) : (
